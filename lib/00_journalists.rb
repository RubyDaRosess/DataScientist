...
def handle_five
    puts "Il y a #{Arr.count {|string| string.length == 6}} handle avec 5 caractères dans cet array"
end

# Combien commencent par une majuscule (première lettre juste après le @) ?
def handle_startwith_upcase
    "Il y a #{Arr.count {|string| string[1] == string[1].upcase if string[1] != '_'}} handles qui commencent par une majuscule dans cet array"
end

# Trie la liste de handle par ordre alphabétique.
def handle_alphabetical_sort
    puts"\nVoici la liste triée par ordre alphabétique"
    Arr.sort_by {|a| a.upcase}
end
...
puts handle_min
puts handle_five
puts handle_startwith_upcase
puts handle_alphabetical_sort
puts handle_length_sort
puts handle_find_epenser
puts handle_repartition